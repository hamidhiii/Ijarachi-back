...
Отдельный от админки сайт для поддержки: видно, где сделка встала и что требует
ручного решения. Свой вход на `/monitor/login/`, данные читаются из базы в момент
открытия страницы.

Разделы: обзор, сделки с поиском по id/телефону/названию, карточка сделки с историей
статусов и платежами, застрявшие сделки, споры, очередь KYC, платежи и сводка ошибок
(расхождения платёж/сделка, эскроу не по статусу, зависшие вебхуки).

### Кто имеет доступ

Суперюзеру открыто всё без настройки, плюс на карточках появляются ссылки в админку — правка
данных остаётся там, где живёт бизнес-логика и валидация. Остальным доступ выдаётся
вручную в разделе «Доступы»: можно завести нового сотрудника с паролем или открыть монитор
уже зарегистрированному пользователю, отметив конкретные разделы. Созданные так учётки не
получают `is_staff`, то есть в `/admin/` не попадают.

### История статусов

`BookingStatusLog` пишется сигналом на `post_save` брони, поэтому фиксируются и переходы через
`transition_to()`, и прямые присваивания `status`/`escrow_status` во вьюхах и задачах Celery.
История начинается с момента миграции: у старых сделок карточка покажет только `created_at`
и `updated_at`. Пороги «застряло» настраиваются переменными `MONITOR_*` в `.env`.
